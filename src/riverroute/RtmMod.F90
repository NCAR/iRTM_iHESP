...
! !USES:
  ...
  use RtmVar          , only : re, spval, rtmlon, rtmlat, iulog, ice_runoff, &
                               ...
                               rtm_active, flood_active, effvel_active, wiso_runoff
...
!-----------------------------------------------------------------------
    ...
    namelist /rtm_inparm / &
         ...
         rtmhist_fincl1,  rtmhist_fincl2, rtmhist_fincl3, &
         rtmhist_fexcl1,  rtmhist_fexcl2, rtmhist_fexcl3, &
         rtmhist_avgflag_pertape, wiso_runoff
    ...
    call mpi_bcast (wiso_runoff,  1, MPI_LOGICAL, 0, mpicom_rof, ier)
    ...
    if (masterproc) then
      ...
      if (wiso_runoff) then
         write(iulog,*) '   RTM :: water isotope runoff is on '
      else
         write(iulog,*) '   RTM :: water isotope runoff is off '
      endif
    endif
